...
class ResNet18(nn.Module):
    ...
    def forward(self, imgs):
        # return self.classifier1(self.modle(imgs)), self.classifier2(self.modle(imgs)), self.classifier3(self.modle(imgs)), self.classifier4(self.modle(imgs)), self.classifier5(self.modle(imgs))
        # return self.classifier1(self.modle(imgs))
        return self.classifier1(self.modle(imgs)), self.classifier2(self.modle(imgs)), self.classifier3(self.modle(imgs))
...
def recognizeAttribute(image):
    ...
    pt_path = '../datasets/3-age_gender_model_79.pth'
    resnet18 = ResNet18()
    ...
    # print(gender,prob1)
    # print(age,prob2)
    # return [[gender,conf1],[age,conf2],[nation,conf3],[height,conf4],[weight,conf5]]
    # 当只测三个标签时
    with torch.no_grad():
        output1,output2,output3= model(input_batch)
        # print(output) #输出五元组
        print("output1,output2,output3,output4,output5")
    ...
    return [[gender,conf1],[age,conf2],[nation,conf3]]

def main():
    imgpath = "/Users/fariy/Desktop/MUC/数字图像处理/期末/身份识别任务数据集/训练集/00024/2023-03-06-0.jpg"
    ...
    print(f"性别 : {attribute[0]}, 年龄 : {attribute[1]}, 民族 : {attribute[2]}")
